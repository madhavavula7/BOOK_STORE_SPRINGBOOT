...
public class AuthServiceImpl implements AuthService {
    ...
    public ApiResponse<AuthResponse> login(LoginRequest req) {
        ...
        // FIXED: Passing 3 arguments: token, role, and message
        AuthResponse authData = new AuthResponse(token, roleName, "Login Successful");

        return new ApiResponse<>(true, "Welcome back, " + user.getName(), authData);
    }

    @Override
    public ApiResponse<AuthResponse> adminLogin(LoginRequest req) {
        User user = repo.findByEmail(req.getEmail())
                .orElseThrow(() -> new RuntimeException("Admin account not found"));
        ...
        // FIXED: Passing 3 arguments here as well
        AuthResponse authData = new AuthResponse(token, roleName, "Admin Login Successful");

        return new ApiResponse<>(true, "Welcome to Admin Dashboard", authData);
    }
